...
TWO_PANEL_DEFAULTS: Dict[str, Any] = {
    "page_width_mm": 245.0,
    "page_height_mm": 250.0,
    "panel_width_mm": 85.0,
    "panel_height_mm": 73.0,
    "gap_mm": 20.0,
    "panel_labels": {
        "enabled": True,
        "font_family": "Times New Roman",
...
def resolve_two_panel_layout(cfg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Center the complete A+B panel group on the physical page.

    No absolute left/top placement is used.  The group bounding box is:
        width  = 2*panel_width + gap
        height = panel_height
    and its center is forced to the page center.
    """
    user = cfg.get("two_panel", {})
    if not isinstance(user, dict):
        user = {}

    layout = deep_resolve(user, TWO_PANEL_DEFAULTS)

    pw = cfg_float(layout.get("page_width_mm"), 245.0)
    ph = cfg_float(layout.get("page_height_mm"), 250.0)
    panel_w = cfg_float(layout.get("panel_width_mm"), 85.0)
    panel_h = cfg_float(layout.get("panel_height_mm"), 73.0)
    gap = cfg_float(layout.get("gap_mm"), 20.0)

    group_w = 2.0 * panel_w + gap
    group_h = panel_h

    if group_w > pw or group_h > ph:
        raise ValueError(
            f"Centered two-panel group ({group_w:g} x {group_h:g} mm) "
            f"cannot fit on page ({pw:g} x {ph:g} mm)."
        )

    group_left = (pw - group_w) / 2.0
    group_top = (ph - group_h) / 2.0

    layout["_left_a_mm"] = group_left
    layout["_left_b_mm"] = group_left + panel_w + gap
    layout["_top_mm"] = group_top
    layout["_group_width_mm"] = group_w
    layout["_group_height_mm"] = group_h
...
def create_origin_worksheet(op, excel_path: Path, sheet_name: str,
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not set Origin XY designations for panel {panel_name}: {e}"
        ) from e
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not verify Origin worksheet for panel {panel_name}: {e}"
        ) from e
...
def add_series_to_layer(layer, wks, series_list: List[SeriesInfo],
    ...
    for i, _ in enumerate(series_list):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to add panel {panel_name} plot {i+1}: {e}"
            ) from e
        if plot is None:
            raise RuntimeError(
                f"Origin returned no plot object for panel {panel_name}, curve {i+1}."
            )
        plots.append(plot)
    ...
    if len(actual) != len(series_list):
        raise RuntimeError(
            f"Panel {panel_name} contains {len(actual)} plots, "
            f"expected {len(series_list)}."
        )

    try:
        layer.rescale()
    except Exception as e:
        raise RuntimeError(f"Panel {panel_name} rescale failed: {e}") from e

    try:
        print(
            f"[ORIGIN SCALE {panel_name}] x={layer.xlim}, y={layer.ylim}"
        )
    except Exception:
        pass
...
def set_two_panel_geometry(op, graph, cfg: Dict[str, Any]) -> Dict[str, Any]:
    layout = resolve_two_panel_layout(cfg)

    page_w = cfg_float(layout.get("page_width_mm"), 245.0)
    page_h = cfg_float(layout.get("page_height_mm"), 250.0)
    panel_w = cfg_float(layout.get("panel_width_mm"), 85.0)
    panel_h = cfg_float(layout.get("panel_height_mm"), 73.0)
    left_a = cfg_float(layout.get("_left_a_mm"), 27.5)
    left_b = cfg_float(layout.get("_left_b_mm"), 132.5)
    top = cfg_float(layout.get("_top_mm"), 88.5)
    ...
    # Same physical-page logic as the V12 single-panel baseline.
    ...
    # One calibration pass, matching the proven single-panel workflow.
    try:
        actual_w = op.lt_float("page.width/page.resx*25.4")
        actual_h = op.lt_float("page.height/page.resy*25.4")
        if actual_w and abs(actual_w - page_w) > 0.05:
            op.lt_exec(f"page.width=page.width*{page_w/actual_w:.12g};")
        if actual_h and abs(actual_h - page_h) > 0.05:
            op.lt_exec(f"page.height=page.height*{page_h/actual_h:.12g};")
    except Exception as e:
        print(f"[WARN] Could not calibrate two-panel page size: {e}")
    ...
    # Fixed Factor = 1 for both layers.
    op.lt_exec("page -afu1;")

    # Read back.
    try:
        actual_page_w = op.lt_float("page.width/page.resx*25.4")
        actual_page_h = op.lt_float("page.height/page.resy*25.4")
        print(
            "[TWO-PANEL GEOMETRY] "
            f"page={actual_page_w:.3f}x{actual_page_h:.3f} mm; "
            f"A=({left_a:.3f},{top:.3f},{panel_w:.3f},{panel_h:.3f}) mm; "
            f"B=({left_b:.3f},{top:.3f},{panel_w:.3f},{panel_h:.3f}) mm; "
            f"gap={left_b-(left_a+panel_w):.3f} mm; "
            f"group_center=({left_a + (2*panel_w + (left_b-(left_a+panel_w)))/2:.3f}, "
            f"{top + panel_h/2:.3f}) mm"
        )
    except Exception as e:
        print(f"[WARN] Could not read back two-panel geometry: {e}")

    return layout


def add_panel_label(op, layer_index: int, text: str,
                    obj_name: str, cfg: Dict[str, Any]) -> None:
    layout = resolve_two_panel_layout(cfg)
    lab_cfg = layout.get("panel_labels", {})
    ...
    # Put label just inside the upper-left corner of each panel.
    try:
        op.lt_exec(
            f'label -p 2 3 -j 0 -n {obj_name} "{text}";'
        )
        op.lt_exec(f"{obj_name}.font=font({font});")
        op.lt_exec(f"{obj_name}.fsize={size};")
        op.lt_exec(f"{obj_name}.bold={1 if bold else 0};")
    except Exception as e:
        print(f"[WARN] Could not create panel label {text}: {e}")


def style_two_panel_layer(op, graph, layer, plots, cfg, series_list,
                          layer_index: int, panel_name: str) -> None:
    op.lt_exec(f"page.active={layer_index};")
    apply_origin_style(op, graph, layer, plots, cfg, series_list)

    # Reuse the satisfactory V12 axis-title/legend behavior independently
    # inside each active layer.
    create_latex_axis_titles(op, cfg, series_list)
    create_extra_latex_legend(op, cfg, series_list)

    actual = layer.plot_list()
    print(
        f"[ORIGIN FINAL CHECK {panel_name}] "
        f"data plots in layer={len(actual)}"
    )
    if len(actual) != len(series_list):
        raise RuntimeError(
            f"One or more plots disappeared from panel {panel_name} during formatting."
        )


def plot_two_panel_in_origin(
    excel_a: Path,
    sheet_a: str,
    excel_b: Path,
    sheet_b: str,
    ...
    # Panel-specific worksheets. They can point to the same Excel/sheet.
    ...
    # Create page + two independent Cartesian layers.
    try:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not add the second Origin graph layer: {e}"
        ) from e

    if len(gp) < 2:
        raise RuntimeError(
            "Origin graph page does not contain two layers after add_layer()."
        )
    ...
    layout = set_two_panel_geometry(op, gp, cfg)

    # Apply V12 styling independently.
    style_two_panel_layer(
        op, gp, layer_a, plots_a, cfg, series_a, 1, "A"
    )
    style_two_panel_layer(
        op, gp, layer_b, plots_b, cfg, series_b, 2, "B"
    )

    # Panel markers.
    label_cfg = layout.get("panel_labels", {})
    add_panel_label(
        op, 1, str(label_cfg.get("a") or "(a)"), "PanelLabelA", cfg
    )
    add_panel_label(
        op, 2, str(label_cfg.get("b") or "(b)"), "PanelLabelB", cfg
    )

    outputs = save_origin_project_and_pdf(
        op, gp, out_dir, figure_name, cfg
    )
    return outputs
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Create a two-panel Origin figure using the stable V12 style. "
            "Panel B defaults to the same Excel file/sheet as panel A."
        )
    )
    parser.add_argument("--excel-a", required=True, help="Excel file for Fig(a)")
    parser.add_argument("--sheet-a", required=True, help="Sheet for Fig(a)")
    parser.add_argument(
        "--excel-b",
        default=None,
        help="Excel file for Fig(b); defaults to --excel-a"
    )
    parser.add_argument(
        "--sheet-b",
        default=None,
        help="Sheet for Fig(b); defaults to --sheet-a"
    )
    parser.add_argument("--config", required=True, help="V12 style YAML")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--figure-name", default="Fig1ab")
    ...
    excel_b = Path(args.excel_b).expanduser().resolve() if args.excel_b else excel_a
    ...
    print(
        "[TWO-PANEL CONFIG] "
        f"page={layout['page_width_mm']}x{layout['page_height_mm']} mm, "
        f"panel={layout['panel_width_mm']}x{layout['panel_height_mm']} mm, "
        f"gap={layout['gap_mm']} mm, "
        f"group_center=({layout['page_width_mm']/2:.3f}, "
        f"{layout['page_height_mm']/2:.3f}) mm"
    )

    # Parse both panels before Origin is called.
    ...
    if args.dry_run:
        print("[DRY-RUN] Both panels parsed successfully. Origin was not called.")
        return

    outputs = plot_two_panel_in_origin(
        excel_a=excel_a,
        sheet_a=sheet_a,
        excel_b=excel_b,
        sheet_b=sheet_b,
