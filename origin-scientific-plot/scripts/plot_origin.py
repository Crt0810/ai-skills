...
def validate_config(cfg: Dict[str, Any]) -> None:
    ...
    # Centering is now a hard layout rule: ignore legacy position offsets.
    ox = 0.0
    oy = 0.0
...
def set_origin_page_and_layer_geometry(op, graph, cfg: Dict[str, Any]) -> None:
    ...
    # Hard rule: the panel itself is always centered on the physical page.
    left_mm = (target_page_w_mm - panel_w) / 2.0
    top_mm = (target_page_h_mm - panel_h) / 2.0
    ...
    # Origin can drift the vertical page size while layer geometry is applied;
    # re-assert the configured physical height before verifying output.
    try:
        op.lt_exec(f"page.height=page.resy*{page_h_in:.12g};")
    except Exception as e:
        print(f"[WARN] Could not re-assert physical page height: {e}")
    ...
    try:
        ...
        print(
            ...
            f"center=({actual_left + actual_width/2:.3f},"
            f"{actual_top + actual_height/2:.3f}) mm"
        )
    except Exception as e:
...
def plot_in_origin(
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install it in the Python that drives your local Origin."
        ) from e
    ...
    # Create workbook and import numeric data
    book_name = f"{figure_name}_Data"
    ...
    # Put clean rectangular data into Origin worksheet
    try:
        wks.from_df(numeric_df, index=False, addindex=False)
    except TypeError:
        wks.from_df(numeric_df)

    # Designate columns as independent XY pairs.
    # originpro documents lower-case letters for plotting designations.
    axis_designation = "".join(["xy" for _ in series_list])
    try:
        wks.cols_axis(axis_designation, c1=0, c2=(2 * len(series_list) - 1), repeat=False)
    except Exception as e:
        raise RuntimeError(f"Could not set Origin worksheet XY designations: {e}") from e

    # Verify that the numerical data actually reached Origin before plotting.
    try:
        origin_df = wks.to_df()
        print(
            f"[ORIGIN WORKSHEET] rows={len(origin_df)}, cols={origin_df.shape[1]}, "
            f"designation={axis_designation}"
        )
        if origin_df.empty or origin_df.shape[1] < 2 * len(series_list):
            raise RuntimeError(
                "Origin worksheet does not contain the expected imported numerical data."
            )
    except Exception as e:
        raise RuntimeError(f"Origin worksheet verification failed: {e}") from e
    ...
    try:
        gp.set_name(graph_name)
    except Exception:
        pass

    layer = gp[0]

    # Add each independent XY pair
    plots = []
    for i, s in enumerate(series_list):
        xcol = 2 * i
        ycol = 2 * i + 1
        try:
            # Official originpro plot-type code: 'l' = Line Plot.
            plot = layer.add_plot(wks, coly=ycol, colx=xcol, type="l")
        except Exception as e:
            raise RuntimeError(
                f"Failed to add Origin line plot {i+1} using columns "
                f"X={xcol}, Y={ycol}: {e}"
            ) from e
        if plot is None:
            raise RuntimeError(f"Origin failed to create plot {i+1}.")
        plots.append(plot)
        try:
            print(f"[ORIGIN PLOT] {i+1}: {plot.lt_range()}")
        except Exception:
            print(f"[ORIGIN PLOT] {i+1}: plot object created")

    # Never claim success if Origin did not actually create all curves.
    actual_plots = layer.plot_list()
    print(f"[ORIGIN PLOT CHECK] expected={len(series_list)}, actual={len(actual_plots)}")
    if len(actual_plots) != len(series_list):
        raise RuntimeError(
            f"Origin created {len(actual_plots)} data plots, "
            f"but {len(series_list)} were expected."
        )
    ...
    try:
        ...
        raise RuntimeError(f"Origin failed to rescale the graph layer: {e}") from e

    try:
        print(f"[ORIGIN SCALE] x={layer.xlim}, y={layer.ylim}")
    except Exception as e:
        print(f"[WARN] Could not read rescaled axis limits: {e}")

    # Apply geometry and style
    set_origin_page_and_layer_geometry(op, gp, cfg)
    apply_origin_style(op, gp, layer, plots, cfg, series_list)

    final_plots = layer.plot_list()
    print(f"[ORIGIN FINAL CHECK] data plots in layer={len(final_plots)}")
    if len(final_plots) != len(series_list):
        raise RuntimeError(
            "One or more data plots disappeared during formatting; "
            "the project will not be reported as successfully plotted."
        )

    # Replace axis titles with LaTeX-style text and add a comparison LaTeX legend block.
    create_latex_axis_titles(op, cfg, series_list)
    create_extra_latex_legend(op, cfg, series_list)

    # Save outputs
    outputs = save_origin_project_and_pdf(op, gp, out_dir, figure_name, cfg)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot standardized Excel data in Origin.")
    parser.add_argument("--excel", required=True, help="Path to the standardized Excel workbook.")
    parser.add_argument("--sheet", required=True, help="Sheet name to use, e.g. Fig1_Standard.")
    parser.add_argument("--config", required=True, help="Path to style.yaml.")
    parser.add_argument("--output-dir", default=".", help="Where to save .opju, .pdf, and manifest.")
    parser.add_argument("--figure-name", default=None, help="Base name of the output figure.")
    parser.add_argument("--dry-run", action="store_true", help="Parse and validate only; do not call Origin.")
    parser.add_argument("--hide-origin", action="store_true", help="Try to keep Origin hidden during automation.")
    ...
    print(
        "[CONFIG] "
        f"canvas={cfg['canvas']['width_mm']}x{cfg['canvas']['height_mm']} mm, "
        f"panel={cfg['single_panel']['width_mm']}x{cfg['single_panel']['height_mm']} mm, "
        f"font={cfg['text']['font_family']}, "
        f"font_size={cfg['text']['axis_title']['size_pt']} pt, "
        f"axis_width={cfg['axes']['line_width_pt']} pt, "
        f"tick_length={cfg['axes']['ticks']['major']['length_pt']} pt, "
        f"curve_width={cfg['curves']['line_width_pt']} pt, "
        f"colors={cfg['curves']['color_sequence']}, "
        f"latex_axis_size={cfg['latex']['axis_titles']['font_size_pt']} pt, "
        f"legend_frame={cfg['text']['legend']['frame']}, "
        f"tick_font={cfg['axes']['tick_labels']['font_family']}, "
        f"tick_size={cfg['axes']['tick_labels']['size_pt']} pt, "
        f"y_latex_size={cfg['latex']['axis_titles']['y_font_size_pt']} pt"
    )
    series_list = parse_standardized_sheet(excel_path, args.sheet)

    figure_name = args.figure_name
    if figure_name is None:
        figure_name = Path(args.sheet).stem

    manifest_path = out_dir / f"{figure_name}_manifest.json"
    write_manifest_json(series_list, manifest_path)

    # Always print a readable summary
    print("=" * 72)
    ...
    for s in series_list:
        print(
            f"Curve {s.curve_id}: "
            f"({s.x_short}, {s.y_short}), "
            f"N_in={s.n_in:g}, "
            f"points={len(s.x)}, "
            f"x=[{np.min(s.x):.6g}, {np.max(s.x):.6g}], "
            f"y=[{np.min(s.y):.6g}, {np.max(s.y):.6g}]"
        )
    print("-" * 72)
    ...
    if args.dry_run:
        print("[DRY-RUN] Parsing succeeded. Origin was not called.")
        return

    outputs = plot_in_origin(
    ...
    print("Origin plotting finished.")
